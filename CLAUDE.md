...
- **CSS cache-busting**: `index.html` references `styles.css?v=N` (currently `v=12`, last bumped on 2026-06-02 when WatchCircle (A1) landed and the lineup was reordered). Bump `N` whenever a CSS change risks hitting stale browser/Fastly caches — Pages' `cache-control: max-age=600` on the file means without a query bump, returning visitors can render unstyled HTML for up to 10 min.
